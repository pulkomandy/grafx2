...
int L_InputBox(lua_State* L)
{
  ...
  // OK
  Window_set_normal_button( 7, 25 + 17 * nb_settings, 51,14,"OK" , 0,1,SDL_SCANCODE_RETURN);
...
int L_WaitInput(lua_State* L)
{
  ...
  lua_pushinteger(L, (Key == KEY_ESC) ? SDL_SCANCODE_ESCAPE : Key);
...
void Button_Brush_Factory(void)
{
  ...
  Window_set_normal_button(10, 161, 67, 14, "Run", 0, 1, SDL_SCANCODE_RETURN); // 5
  ...
  while (1)
  {
    ...
    {
      clicked_button = Window_clicked_button();
      if (Key==SDL_SCANCODE_BACKSPACE && Config.Scripts_directory[0]!='\0')
      {
